  ...
  */
                                   ...
                                   int *coarse_to_fine, const Arg &arg)
  {
    ...
    if constexpr (rescale) {
      ...
      auto process_max = [&](int, int, complex<store_t> a[elements_per_thread], float scale_inv_) {
        if constexpr (gmem_obj_t::fixed) {
          thread_max = fmaxf(fixedMaxValue<store_t>::value * scale_inv_, thread_max);
        } else {
#pragma unroll
      ...
      loop_over<contiguous_dim, contiguous_limit, elements_per_thread>(
        gmem, x_coarse, coarse_spin, contiguous_dim_offset, aggregate_k_offset, coarse_to_fine, arg, process_max);
    ...
    auto write_to_smem = [&](int smem_m, int smem_k, complex<store_t> a[elements_per_thread], float scale_inv_) {
      ...
      if constexpr (gmem_obj_t::fixed) {
        auto scale_inv = scale_inv_ * block_rescale_factor;
#pragma unroll
        for (int e = 0; e < elements_per_thread; e++) {
...
#pragma unroll
  ...
  void __device__ inline restrict_mma(int x_coarse, int coarse_spin, int m_offset, int n_offset, const Arg &arg)
  {
    ...
    int aggregate_k_offset_limit = (Arg::fineSpin == 1 ? static_cast<int>(arg.aggregate_size_cb) : arg.aggregate_size);
    for (int aggregate_k_offset = 0; aggregate_k_offset < aggregate_k_offset_limit;
